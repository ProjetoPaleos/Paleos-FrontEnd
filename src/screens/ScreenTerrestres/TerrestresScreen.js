...
export default function TerrestresScreen({ navigation }) {
  ...
  const fetchFosseis = async () => {
    setLoading(true);
    try {
      const response = await api.get('/terrestre');
      setFosseis(response.data);
    } catch (error) {
      console.error('Erro ao buscar fósseis terrestres:', error);
    }
    setLoading(false);
  };
  ...
  const renderItem = ({ item }) => (
    <CardFossilMarinho
      ...
      onPress={() => navigation.navigate('ViewDetailsTerrestre', { id: item.id })}
    />
  );
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        <Text style={styles.titulo}>Fósseis terrestres</Text>
        <TouchableOpacity onPress={() => navigation.navigate('AddScreenTerrestre')}>
